...
impl NumericalIntegration {
    ...
    //     let gauss = values_gauss_rule(gauss_point_numbers);

    //let mut vec_weight:Vec<f64>=Vec::new();
    //    let vec_weight: Vec<_> = gauss.iter().map(|p| p.weigth.clone()).collect();

    //println!("Weghts = {:?}", vec_weight);
    //}
...
// fn values_gauss_rule(gauss_point_numbers: u32) -> Vec<NumericalIntegration> {
...
//     let my_gauss_values = search_gauss_values(values, gauss_point_numbers);
//     println!("gauss values = {:?}", my_gauss_values);
